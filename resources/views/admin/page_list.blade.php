...
<div class="page-body">
  	<div class="container-xl">
		<div class="card">
      		<div class="card-body">
				@if(session('page_delete_success'))
				<div class="alert alert-success"><strong>Success!</strong> {{session('page_delete_success')}}</div>
			  @endif
			  @if(session('page_delete_error'))
				<div class="alert alert-danger"><strong>Error!</strong> {{session('page_delete_error')}}</div>
			  @endif   
							...
							@foreach($pages as $p)
								<tr>
									...
									<td class="text-center">
										...
										<form method="POST" action="{{ route('page.delete') }}" id="form-page-delete-{{$p->page_id}}">
											@csrf
